...
def data_collection(num_iters, plot_sol):
    ...
    for iter in range(num_iters):
        ...
        start = [0.0,random.uniform(-0.9,0.9), 0.0, 0.0]
...
def dagger_problem(save_data=True):
...
def relaxed_problem():
    ...
    while dist(goal) > world.TOL:
        ...
        if not isinstance(state_sol, np.ndarray):
            ...
            if abs(dist(goal)-prev_dist)<1e-5:             # If the robot is stuck, use full solution
                print("\nDEBUG: Stuck, solving the full problem")
                f_state0.value = np.array(robot.state)
                f_goal0.value  = np.array(goal)
                f_lower_obs.value = np.array(lower_cpy)
                f_upper_obs.value = np.array(lower_cpy) + np.array(size_cpy)
...
if __name__ == "__main__":
    ...
    if SOLVE:
        data_collection(num_iters=1, plot_sol=True)
    elif DAGGER:
        # relaxed_problem(dataset, use_model=True, do_dagger=True)
        dagger_problem()
    else:
        relaxed_problem()